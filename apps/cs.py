...
def app():
    ...
    def load_data():
        ...
        df = df.drop(['PROVINCE_C', 'BARANGAY_C', 'BARANGAY_A', 
                    'MUNCITY', 'MUNCITY__1', 'MUNCITY_NA', 'URBAN_RU_1', 
                    'REGION', 'MUNCITY_AL', 'fid_', 'MUNCITY_CO', 'REGION_COD', 
                    'PROVINCE', 'MUNCITY_TY'], axis=1)

        df = pd.melt(df, id_vars=['system:index', 'BARANGAY', 'URBAN_RURA', 'Area', 'POPULATION'], )
        df.variable = df.variable.apply(lambda x: x[12:20])
        df = df.rename(columns={'value':'NDVI', 'variable':'Timestamp', 
                                'system:index':'Default', 'BARANGAY':'Barangay',
                                'URBAN_RURA':'Class', 'POPULATION':'Population'})
    ...
    brgy_select = st.multiselect('Select Barangay', df['Barangay'].unique(), df['Barangay'].unique()[:2])
    ...
    with metric1:
        ...
        st.metric(f'Mean NDVI', f'{ndvi_brgy["NDVI"].mean():0.3f}', f'Selected: {len(brgy_select)}', delta_color='off')
    ...
    with metric4:
        ...
        st.metric(f'Landsat 8 Images', f'{num_file}', f'~{0.919*num_file:0.2f} GB', delta_color='off')
    ...
    altA = alt.Chart(df1).mark_line(point=True).properties(width=750).encode(
        x=alt.X('Timestamp:T'),
        y=alt.Y('NDVI:Q', scale=alt.Scale(domain=[0,1])),
        color=alt.Color('Barangay:N', legend=alt.Legend(orient='bottom')),
        tooltip=[alt.Tooltip('Barangay:N'), alt.Tooltip('Timestamp:T'), alt.Tooltip('NDVI:Q')]).interactive()

    st.altair_chart(altA, use_container_width=True)

    altB = alt.Chart(ndvi_brgy).mark_bar(point=True).properties(width=750).encode(
    ...
    df1.columns = ['Default', 'Barangay', 'Class', 'Area', 'Population', 'Timestamp', 'NDVI']
