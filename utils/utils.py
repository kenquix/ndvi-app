...
def read_data(l8, startdate, enddate, aoi, datamask, scale):
    ...
    classes = ["Soil/Water", "Very Low", "Low", "Mod. Low", "Mod. High", "High"]
    ...
    try:
        ...
        st.error(
            "Too many pixels in sample; must be <= 262144. Please select a smaller region or decrease resolution (i.e. Increase the value of scale.)."
        )
        return
    ...
    hist_df = pd.DataFrame(
        {f"{start1}": start_arr.flatten(), f"{end1}": end_arr.flatten(),}
    )
    ...
    interpretation = [
        ...
        for i in list(perc_change)
    ]

    tabular_df = pd.DataFrame(
        {
            f"Area ({startdate})": start_bin_norm * area,
    ...
    df = df.rename(
        columns={"value": "NDVI", "variable": "Timestamp", "system:index": "Default"}
    )
    ...
    df["NDVI_Lowess"] = lowess(
        df.NDVI.values, np.arange(len(df.NDVI.values)), frac=0.05
    )[:, 1]
    ...
    return (
        df,
        tabular_df,
        start_img.updateMask(datamask),
        end_img.updateMask(datamask),
        diff_img.updateMask(datamask),
        diff_bin_norm,
        hist_df,
    )
...
def ruler(data, field, field_type, base_chart, text_value):
    nearest = alt.selection(
        type="single", nearest=True, on="mouseover", fields=[f"{field}"], empty="none",
    )
